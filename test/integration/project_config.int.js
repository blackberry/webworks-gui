...
describe("project_config", function () {
    ...
    describe("get", function () {
        it("returns content of config.xml", function () {
            var cb = jasmine.createSpy().andCallFake(function (res, body) {
                    var result = JSON.parse(body);
                    expect(result.success).toBeTruthy();
                    expect(result.configFile).toEqual(jasmine.any(Object));
                });

            testUtil.httpGet(testUtil.getTestURL("project_config", {
    ...
    describe("put", function () {
        it("writes specified content to config.xml", function () {
            var cb = jasmine.createSpy().andCallFake(function (res, body) {
                    var result = JSON.parse(body),
                        actualFile;
                    expect(result.success).toBeTruthy();
                    actualFile = fs.readFileSync(path.join(FULL_VALID_PROJECT_PATH, "www", "config.xml"), {
                        encoding: "utf8"
                    });
                    expect(actualFile).toEqual("abcd");
                }),
            ...
            testUtil.httpPost(options, postData, cb);
