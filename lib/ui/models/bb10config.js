...
BB10Config = Backbone.Model.extend({
    ...
    getProxyInfo: function (callback) {
        var proxyInfo = {},
            responseHandler = function (response) {
                if (!response.error) {
                    proxiedApps.forEach(function (app) {
                        if (response.config[app]) {
                            proxyInfo[app] = response.config[app];
                        }
                    });
                    callback(proxyInfo);
                }
        ...
        this.getBB10Config(responseHandler);
    },
    targetFilter: function (deviceFlag, callback) {
        var newTargetList = {},
            filter = function (response) {
                for (var target in response.config.targets) {
                    if (!deviceFlag) {
                        if (target.type === "simulator" || target.type === "emulator") {
