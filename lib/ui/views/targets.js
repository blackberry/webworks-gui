...
configOrFieldSetter = function (bb10config, targetName, action) {
    ...
    bb10config = bb10config || {};
    bb10config.targets = bb10config.targets || {};
...
TargetListView = Backbone.View.extend({
    ...
    render: function () {
        ...
        this.model.getBB10Config(function (response) {
            ...
            if (!response.error) {
                response.config.targets = response.config.targets || {};
                ...
                alert("Error accessing file at " + response.error.path);
            }
        });
    },
    events: {
