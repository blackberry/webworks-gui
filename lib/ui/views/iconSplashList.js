...
IconSplashListView = Backbone.View.extend({
    ...
    addToConfig: function (event) {
        ...
        server.resourceExists({
            ...
            if (response.exists && response.isMatched) {
                updateIconOrSplash(config, filePath, that.options.type, that.iconDisplay);
                ...
                alert("Invalid path: " + response.error);
            }
        });
    },
    removeFromConfig: function (event) {
        ...
        deleteIconOrSplash(config, event.currentTarget.getAttribute("data-src"), this.options.type, this.iconDisplay);
        this.model.set("config", config);
        window.views.projectInfo.dirty = true;
        this.render();
    },
    validate: function () {
        var targetSelector = this.options.type === "icon" ? "#f-new-icon-entry" : "#f-new-splash-entry",
        validator = $(targetSelector).parsley();
        validator.validate();
