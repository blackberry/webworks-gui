...
View = Backbone.View.extend({
    initialize: function () {
        ...
        $.get("pages/proxies.html", function (data) {
            viewTemplate = data;
        });
    },
    render: function () {
        ...
        this.model.getTargetConfig(function (response) {
            ...
            if (!response.error) {
                ...
                proxiedApps.forEach(function (app) {
                    ...
                    Object.getOwnPropertyNames(settings).forEach(function (p) {
                        var dashp = '-proxy' + p,
                            k,
                            v;
                ...
                Object.getOwnPropertyNames(settings).forEach(function (p) {
                    var value = settings[p],
                        valuemap = [['', 0]];
                    Object.getOwnPropertyNames(value).forEach(function (cand) {
    ...
    events: {
        "click #saveProxy" : "saveProxy"
    },
    saveProxy: function (event) {
        var view = this,
            settings = {
                host: $("#proxyhost").val(),
                port: $("#proxyport").val(),
            ...
            config = this.model.get("targetConfig");
        ...
        proxiedApps.forEach(function (app) {
            ...
            if (Object.getOwnPropertyNames(appsettings)) {
                config[app] = appsettings;
            } else {
                delete config[app];
            }
        });
        this.model.setTargetConfig(function (output) {
        ...
        // Display dialog
        window.views.dialogBox.display({body: "Updating proxy configuration..."});
    },
