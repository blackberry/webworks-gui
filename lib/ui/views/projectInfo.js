...
var ProjectInfoView,
    ...
    UpdatesListView = require("./updatesList"),
...
function preparePrefs(config) {
    ...
    config.widget.preference.forEach(function (pref) {
        ...
        } else if (pref.name === "DiskCache") {
            prefs.diskCache = (pref.value === "enable") ? "true" : "false";
        ...
        } else if (pref.name === "HideKeyboardFormAccessoryBar") {
...
function renderSubviews(model) {
    ...
    window.views.updatesList.setElement($("#updates"));
    window.views.updatesList.display(model);
...
ProjectInfoView = Backbone.View.extend({
    initialize: function () {
        ...
        window.views.updatesList = new UpdatesListView();
    ...
    display: function (model) {
        this.model = model;
        window.views.projectList.setActiveProject(model);
        this.render();
    },
    events: {
    ...
    updateConfig: function (event) {
        ...
        if (type === "appId") {
            config.widget.id = element.value;
        ...
        } else if (type === "diskCache") {
            updatePref(config, "DiskCache", element.checked ? "enable" : "disable");
    ...
    saveConfigToServer: function () {
        ...
        model.setConfigXml(function (result) {
            view.dirty = false;
            alert(result.success ? "Successfully saved" : result.error);
            if (result.success) {
                model.set("projectId", model.get("config").widget.id);
                model.sync("update");
            }
            window.views.projectList.render();
            view.render();
        });
    }
});
