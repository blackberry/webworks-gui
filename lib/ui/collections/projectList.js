    ...
    initialize: function () {
        ...
        for (projectId in projects) {
            if (projects.hasOwnProperty(projectId)) {
                ...
                projects[projectId].timestamp = new Date(projects[projectId].timestamp);
        ...
        var collection = this,
            ...
            deleteCallback = function (response) {
                if (response.status === 410) {
                    alert(collection.get(modelCid).attributes.id + " no longer exists on the file system. It will be removed from the project list.");
